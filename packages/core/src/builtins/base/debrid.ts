...
export abstract class BaseDebridAddon<T extends BaseDebridConfig> {
  ...
  protected buildQueries(
    ...
    // Independent of the filter above: some indexers only match the apostrophe
    // form ("Madea's"), so re-clean while keeping them when asked.
    if (keepApostrophes) {
      titles = titles.map((t) =>
        cleanTitle(t, undefined, { keepApostrophes: true })
      );
    }
