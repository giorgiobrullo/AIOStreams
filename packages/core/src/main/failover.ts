...
function buildPlaybackInfo(
  ...
  return fileInfo.type === 'torrent'
    ? {
        ...
        // Prowlarr can only give us a placeholder hash for some indexers; the
        // real one is resolved later from the .torrent. Carry it through so the
        // qBittorrent store can match against what it already has.
